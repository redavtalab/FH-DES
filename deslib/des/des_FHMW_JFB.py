...
class DESFHMW_JFB(BaseDES):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        super(DESFHMW_JFB, self).__init__(pool_classifiers=pool_classifiers,
                                    with_IH=with_IH,
                                    safe_k=safe_k,
                                    IH_rate=IH_rate,
    ...
    def fit(self, X, y):
        ...
        super(DESFHMW_JFB, self).fit(X, y)
...
###
    ...
    def FMM_train(self, classifier_ind):
        ...
        for ind, x in enumerate(X):
            ...
            nearest_box = None
            for box in activeBoxes:
                dst = np.linalg.norm(X - box.Center)
                if dst > nDist:
                    nearest_box = box
                    nDist = dst
            ...
            activeBoxes.append(newBox)
            # if activeBoxes:
            #     self.plot_hyperboxes(activeBoxes)
