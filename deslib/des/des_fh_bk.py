    ...
    def estimate_competence(self, query,neighbors=None, distances=None,
        ...
        for index , sample in enumerate(query):
            ...
            for box in DESFH.HBoxes:
                ...
                if clr!=currentClr:
                    listboxComp.sort()
                    if len(listboxComp)>2:
                        competences_[index,currentClr] = ( 4 * listboxComp[-1] + 3 * listboxComp[-2] + 2 * listboxComp[-3])/9  #+ 3* listboxComp[-4])/24
                    else:
            ...
            listboxComp.sort()    
            if len(listboxComp)>2:
                competences_[index,clr] = ( 4 * listboxComp[-1] + 3 * listboxComp[-2] + 2 * listboxComp[-3])/9  #+ 3* listboxComp[-4])/24
            else:
                competences_[index,clr] = listboxComp[-1] # np.average(listboxComp)    
#            comp = box.membership(sample)
#            listboxComp.append(comp)
        competences_=(np.sqrt(len(sample)) -competences_) * self.k      
