...
class FHDES_Allboxes_GPU(BaseDES):
    ...
    def __init__(self, pool_classifiers=None,
        ...
        super(FHDES_Allboxes_GPU, self).__init__(pool_classifiers=pool_classifiers,
                                    with_IH=with_IH,
                                    safe_k=safe_k,
                                    IH_rate=IH_rate,
    ...
    def fit(self, X, y):
        ...
        super(FHDES_Allboxes_GPU, self).fit(X, y)
    ...
    def estimate_competence(self, query, neighbors=None, distances=None, predictions=None):
        ...
        boxes_classifier = torch.cuda.FloatTensor(len(self.HBoxes),1).fill_(1)
        boxes_W = torch.cuda.FloatTensor(len(self.HBoxes),self.n_features_).fill_(0)
        boxes_V = torch.cuda.FloatTensor(len(self.HBoxes),self.n_features_).fill_(0)
        boxes_center = torch.cuda.FloatTensor(len(self.HBoxes),self.n_features_).fill_(0)
        if self.mis_sample_based:
            competences_ = torch.cuda.FloatTensor(len(query), self.n_classifiers_).fill_(1)
        else:
            competences_ = torch.cuda.FloatTensor(len(query), self.n_classifiers_).fill_(0)
        for i in range(len(self.HBoxes)):
            boxes_classifier[i] = self.HBoxes[i].clsr
            boxes_W[i] = torch.from_numpy(self.HBoxes[i].Max)
            boxes_V[i] = torch.from_numpy(self.HBoxes[i].Min)
            boxes_center[i] = torch.from_numpy((self.HBoxes[i].Max + self.HBoxes[i].Min) / 2)
        boxes_W = boxes_W.reshape(self.NO_hypeboxes, 1, self.n_features_).cuda()
        boxes_V = boxes_V.reshape(self.NO_hypeboxes, 1, self.n_features_).cuda()
        boxes_center = boxes_center.reshape(self.NO_hypeboxes,1,self.n_features_).cuda()
        Xq = torch.from_numpy(query).cuda()
        Xq = Xq.view(1,len(query),self.n_features_)

        ## Membership Calculation
        halfsize = ((boxes_W - boxes_V) / 2).reshape(self.NO_hypeboxes,1,self.n_features_)
        d= torch.abs(boxes_center - Xq) - halfsize
        d[d<0] = 0
        ...
        classifiers, indices, count = torch.unique(boxes_classifier, return_counts = True,return_inverse = True)
        k = 0
        for clsr in classifiers:
            ...
            if len(c_range) > 1:
                ...
                bb_indexes = torch.argsort(-clsrBoxes_m, axis=0)
                b1 = bb_indexes[0,:]
                b2 = bb_indexes[1,:]
                for i in range(0,len(query)):
                    if clsrBoxes_m[b1[i],i]==1 : # if the query sample is located inside the box
        ...
        #### was mistake ####
        if self.mis_sample_based:
            competences_ = torch.max(competences_) - competences_
            # competences_ = np.sqrt(self.n_features_)  - competences_

        scaler = preprocessing.MinMaxScaler()
        competences_ = competences_.cpu().numpy()
